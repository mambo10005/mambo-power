...
| `Branch` with `kind == "transformer"` | `Transformer(model="pi")` | `r`, `x`, `b` per unit on the transformer's own `s_nom` (impedances `r`, `x` `× s_nom / base_mva`; the admittance `b` `× base_mva / s_nom`), `tap_ratio`, `tap_side = 0` (mambo's tap is on the from side), `phase_shift` in degrees. |
