"""Rating-derivation test helper (W3, AC-4/AC-6 fixture half).

No MATPOWER-shipped OPF fixture carries a real ``RATE_A`` (record/m3-research.md §6: every
branch of all five OPF fixtures reads ``RATE_A == 0``, MATPOWER's "unlimited" convention) — so
R3's PTDF flow-limit LP rows and R4's N-1 "violates a limit" check have nothing to bind against
on real multi-bus data. This module derives a synthetic rating *at test time* from each
fixture's own unmodified base-case DC dispatch, the same "documented, test-time transformation
of an already-owned fixture" pattern :mod:`tests._brute_force_lodf` already uses (spec Design
item 3) — no new fixture data is committed.
